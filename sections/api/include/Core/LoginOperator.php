...
class LoginOperator
{
     ...
     * @return int
     * 0: success login
     * 1: password wrong (sitter 1).
     * 2: password wrong (sitter 2).
     * 3: password wrong.
     *
     */
    public function checkLogin($password, $result)
    {
        ...
        if ($result['type'] == 1) {
            $sit1Password = $this->getSitterPassword($result['row']['sit1Uid']);
            if ($sit1Password && password_verify($password, $sit1Password)) {
                return 1;
            }
            $sit2Password = $this->getSitterPassword($result['row']['sit2Uid']);
            if ($sit2Password && password_verify($password, $sit2Password)) {
                return 2;
            }
        }
        return 3;
