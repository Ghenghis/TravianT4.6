...
class ServersCtrl extends ApiAbstractCtrl
{
    ...
    private function serverRowToData($row)
    {
        ...
        // PostgreSQL lowercases column names, so use lowercase keys
        ...
        $server = [
            ...
            "start" => (int)$startTime,
            'secondsPast' => time() - $startTime,
            'fireAndSand' => substr($worldId, -2) == 'fs',
            'tournament' => substr($worldId, -2) == 'tt',
            'activationRequired' => $row['activation'] == 1,
            'hidden' => $row['hidden'] == 1,
            'registerClosed' => $registerClosed == 1,
