...
class RegisterCtrl extends ApiAbstractCtrl
{
    ...
    {
        global $globalConfig;
        $needs = ['gameWorld', 'activationCode', 'password'];
        foreach ($needs as $k) {
            if (!isset($this->payload[$k])) {
                throw new MissingParameterException($k);
            }
        ...
        // DEVELOPMENT MODE: reCAPTCHA validation disabled for testing
        // TODO: Re-enable for production deployment
        /*
        if (!isset($this->payload['captcha'])) {
            throw new MissingParameterException('captcha');
        }
        ...
        */
        ...
        if ($activation) {
            ...
            $worldId = isset($activation['worldid']) ? $activation['worldid'] : $activation['worldId'];
            $refUid = isset($activation['refuid']) ? $activation['refuid'] : $activation['refUid'];
            $server = Server::getServerById($worldId);
            $serverDB = ServerDB::getInstance($server['configFileLocation']);
            $token = ActivateHandler::addActivation($activation['name'], $password, $activation['email'], $refUid, $serverDB);
            $this->response['success'] = true;
            // Redirect to game activation handler
    ...
    public function register()
    {
        ...
        // Check if per-world database config exists
        $serverDB = null;
        if (!empty($server['configFileLocation']) && is_file($server['configFileLocation'])) {
            try {
                $serverDB = ServerDB::getInstance($server['configFileLocation']);
            } catch (\Exception $e) {
                // Per-world database not configured yet, will use global checks only
                $serverDB = null;
            }
        }
    ...
    private function doesNameExists($activeServerId, $serverDB, $playerName)
    {
        {
        ...
        // Skip per-world database checks if not configured
        if ($serverDB !== null) {
            {
    ...
    private function doesEmailExists($activeServerId, $serverDB, $email)
    {
